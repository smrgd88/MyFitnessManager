	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				...
				<h1 class="text-center my-4"><a href="${contextPath}/boards/list?board_no=${bgno }">${board.bgname }</a></h1>
		...
		<div class="col-lg-12">
				<form id='searchForm' action="${contextPath}/boards/list" method='get'>
					<select name='type'>
						<option value="">--</option>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
				...
				function showList(page) {
					getList(
							{page: page||1, type:type,keyword:keyword,bgno:bgno},
							function(cnt, list) {	
								...
								for (var i = 0, len = list.length||0; i<len; i++) {
									str+="<tr>";
									str+="<td>"+(cnt-i-((pageNum-1)*5.0))+"</td>";
									str+="<td><a class='move' href='get?bno="+list[i].bno+"&type="+type+"&keyword="+keyword+"&pageNum="+pageNum+"'>"+list[i].btitle+"</td>";
									str+="<td>"+list[i].bwriter+"</td>";
									str+="<td>"+displayTime(list[i].bdate)+"</td>";
									str+="<td>"+list[i].breadCount+"</td>";
									str+="</tr>";
								}
								boardUL.html(str);
				...
				$("#regBtn").on("click", function() {
					...
					self.location = "${contextPath}/boards/register?board_no="+${bgno };
				...
				function getList(param, callback, error) {
				    ...
				    $.getJSON("${contextPath}/boards/list/" + page +".json"+"?type="+type+"&keyword="+keyword+"&bgno="+bgno,
				        function(data) {
				          if (callback) {
						...
						success: function(result){
							...
							var str ="<b>연관 검색어</b><br/>";
								for (var i = 0; i < result.length; i++) {
									str +='<span>'+result[i].btitle+'</span><br/>';
								}
								$("#results").html(str);
							}else{ 
								$("#results").html("");
